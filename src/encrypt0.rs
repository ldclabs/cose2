...
///
/// The full IV must be present in the unprotected header (label
/// [`HeaderParameterIV`](crate::iana::HeaderParameterIV)) before encrypting;
/// this crate does not generate IVs (it has no RNG dependency).
///
...
impl Encrypt0Message {
    ...
    /// Reads the full IV from the unprotected header, checking its length.
